...
Usage::
    ...
    python -m benchmarks.ablation [--seqlen 2048] [--output results/ablation.csv]
...
from attnfuse.runtime.dispatch import _alibi_slopes, _placeholder_slopes, _placeholder_bias, triton_cdiv
...
def _make_kernel(mask_kind: MaskKind, bm: int, bn: int, nw: int, ns: int) -> TiledKernel:
    cfg = TileConfig(
    ...
    return TiledKernel(
        ...
        config=cfg,
        cache_key=f"ablation_{mask_kind.value}_{bm}_{bn}_{nw}_{ns}",
    )
...
def main() -> int:
    ...
    writer.writerow(["variant", "BLOCK_M", "BLOCK_N", "num_warps", "num_stages", "tflops"])

    print(f"# Ablation sweep  N={N}  device={torch.cuda.get_device_name(0)}")
