...
On an NVIDIA RTX~3090 with fp16 inputs, AttnFuse reaches 83--86\% of
PyTorch SDPA's throughput for dense and causal attention.
For variants not natively supported by SDPA---sliding-window (W=256) and
causal+ALiBi---AttnFuse achieves $38\times$ and $17\times$ speedup at
...
\midrule
\multirow{3}{*}{Causal}
  & 1024 &  63.9 &  76.7 & 0.83$\times$ \\
  & 2048 &  89.8 & 108.0 & 0.83$\times$ \\
  & 4096 & 105.0 & 121.8 & 0.86$\times$ \\
\midrule
\multirow{3}{*}{SW (W=256)}
  & 1024 &  65.2 &   9.4 &  6.9$\times$ \\
  & 2048 & 148.9 &   6.2 & 24.1$\times$ \\
  & 4096 & 298.5 &   7.8 & 38.5$\times$ \\
\midrule
\multirow{3}{*}{Causal+ALiBi}
  & 1024 &  59.5 &   7.2 &  8.2$\times$ \\
  & 2048 &  86.5 &   4.6 & 18.9$\times$ \\
  & 4096 &  97.7 &   5.6 & 17.4$\times$ \\
\bottomrule
\end{tabular}
\end{table}

For dense and causal attention, SDPA's hand-optimised CUTLASS backend
provides 16--20\% better TFLOPS than AttnFuse's Triton kernel.  This gap
...
these variants, achieving 38$\times$ speedup at $N{=}4096$.
...
83--86\% of SDPA's throughput---a modest penalty for kernel generality.
For the two variants not supported (sliding-window and causal+ALiBi),
AttnFuse provides up to $38\times$ speedup by avoiding the $\mathcal{O}(n^2)$
