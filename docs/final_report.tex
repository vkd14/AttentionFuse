...
On an NVIDIA RTX~3090, AttnFuse matches or exceeds PyTorch's
\texttt{flex\_attention} (PyTorch 2.5, the strongest published competitor)
on 12 of 16 (variant, sequence-length) head-to-head cells, and beats it
at every sequence length on sliding-window attention. AttnFuse achieves
up to \textbf{56.5$\times$} speedup over PyTorch SDPA for sliding-window
and \textbf{20.7$\times$} for causal+ALiBi at $N{=}4096$, where SDPA
falls back to $\mathcal{O}(n^2)$. The system's most distinctive
contribution is a novel fused Rotary Position Embedding (RoPE) kernel:
because \texttt{flex\_attention}'s \texttt{score\_mod} hook runs after
the $Q K^\top$ matmul, it structurally cannot fuse pre-score
transformations like RoPE. AttnFuse rotates Q and K tiles inside the
inner loop and achieves up to \textbf{2.59$\times$ speedup} over
\texttt{flex\_attention} on \texttt{causal+RoPE} and \textbf{2.39$\times$}
on \texttt{causal+RoPE+ALiBi}. All five attention variants are verified
by 25 GPU unit tests and an 80-example Hypothesis-driven property fuzzer.
\end{abstract}
...
The denominator uses the dense $4 B H N^2 D$ formula for all variants,
so the SW row reports an \emph{equivalent} dense throughput.
Variant-correct TFLOPS appear in the roofline analysis (\S\ref{sec:roofline}).}
\label{tab:fp16_gpt2}
\small
\begin{tabular}{lrrrr}
\toprule
...
\midrule
\multirow{3}{*}{Causal}
  & 1024 &  85.3 &  76.7 & 1.11$\times$ \\
  & 2048 & 106.2 & 108.0 & 0.98$\times$ \\
  & 4096 & 116.0 & 121.3 & 0.96$\times$ \\
\midrule
\multirow{3}{*}{SW (W=256)}
  & 1024 &  99.1 &   9.4 & 10.6$\times$ \\
  & 2048 & 222.7 &   6.2 & 35.9$\times$ \\
  & 4096 & 439.6 &   7.8 & \textbf{56.5}$\times$ \\
\midrule
\multirow{3}{*}{Causal+ALiBi}
  & 1024 &  86.2 &   7.2 & 12.0$\times$ \\
  & 2048 & 105.9 &   4.6 & 23.0$\times$ \\
  & 4096 & 116.6 &   5.6 & \textbf{20.7}$\times$ \\
\bottomrule
\end{tabular}
\end{table}

For dense and causal attention, AttnFuse now matches SDPA within
4--7\% --- the documented Triton-vs-CUTLASS gap.

For sliding-window and causal+ALiBi, SDPA falls back to an
$\mathcal{O}(n^2)$ computation path. AttnFuse is the only
\emph{sub-quadratic} option that supports these variants through
PyTorch's high-level dispatch, achieving \textbf{56.5$\times$} speedup
on sliding-window and \textbf{20.7$\times$} on causal+ALiBi at $N{=}4096$.

\subsection{Direct Comparison vs PyTorch \texttt{flex\_attention}}
\label{sec:flex}

\texttt{flex\_attention} (PyTorch 2.5+) is the closest published
competitor. It accepts a user-supplied \texttt{score\_mod} function and
a \texttt{BlockMask} structure and emits a fused Triton kernel via
TorchInductor's autotuner. Table~\ref{tab:flex_headtohead} shows the
direct head-to-head with bidirectional sliding window
(matching \texttt{af.sliding\_window} semantics; $W=256$).

\begin{table}[H]
\centering
\caption{Speedup vs.\ \texttt{flex\_attention} (PyTorch 2.5+).
Values $>$1 indicate AttnFuse is faster. Cells in bold are wins.}
...
sequence length. The losses (causal and ALiBi at $N{=}4096$) are within
4--6\% and reflect Triton's compile-time tile choice not matching
Inductor's autotuner-discovered tile shape.

\subsection{Structural Novelty: Compositions \texttt{flex\_attention} Cannot Fuse}
\label{sec:composition}

\texttt{flex\_attention}'s \texttt{score\_mod} hook runs \emph{after} the
$QK^\top$ matmul, so any positional encoding that must be applied to
$Q$ or $K$ before the matmul cannot be fused. RoPE is the canonical
example. \texttt{flex\_attention} must apply RoPE host-side as two
separate kernel launches, incurring an extra HBM round-trip for the
rotated $Q$ and $K$.

Table~\ref{tab:composition} reports the AttnFuse speedup over
\texttt{flex\_attention} on the four compositions of interest.

\begin{table}[H]
\centering
\caption{Speedup over \texttt{flex\_attention} on causal compositions.
For RoPE variants \texttt{flex\_attention} pre-processes Q and K with
two extra kernels. \textbf{Bold} = AttnFuse wins.}
...
\textbf{causal + RoPE}            & \textbf{2.59}$\times$ & \textbf{1.67}$\times$ & \textbf{1.29}$\times$ & \textbf{1.07}$\times$ \\
\textbf{causal + RoPE + ALiBi}    & \textbf{2.39}$\times$ & \textbf{1.70}$\times$ & \textbf{1.23}$\times$ & \textbf{1.08}$\times$ \\
\bottomrule
\end{tabular}
\end{table}

For the RoPE compositions AttnFuse wins at \emph{every} sequence length,
with up to \textbf{2.59$\times$} speedup at $N{=}512$ where the extra
HBM round-trip dominates. As $N$ grows the attention kernel itself
amortises the rotation cost, but the gap remains positive even at
$N{=}4096$. This is the paper's central empirical claim: AttnFuse's
combinator algebra enables fusions that no compile-based attention API
currently provides.
...
\paragraph{Note on profiling methodology.}
Nsight Compute (\texttt{ncu}) was unavailable on the evaluation host, so
tensor-core utilisation and HBM bandwidth in
Figure~\ref{fig:roofline} are computed analytically from measured latency:
\textit{TFLOPS} divides the variant-correct FLOP count
($4BHN^2D$ dense/causal,
$4BHN\,\min(2W,N)\,D$ sliding-window) by latency, and \textit{HBM bandwidth}
sums the analytical Q/K/V/O traffic
(scaled by the effective key fraction $\min(2W,N)/N$ for sliding-window).
The HBM figure is a lower bound that excludes SMEM spills and pipeline
overhead; absolute counter-based numbers from \texttt{ncu} would be
slightly higher but the relative shape of the plot is unchanged.
...
\midrule
dense         & 1824.6 & 144 & 12637$\times$ \\
causal        &  378.0 &  98 &  3866$\times$ \\
sliding\_w256 &  807.4 &  96 &  8388$\times$ \\
causal\_alibi &  396.4 &  96 &  4118$\times$ \\
causal\_rope  &  984.1 & 158 &  6220$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Block-sparse structure.}
The original proposal listed block-sparse attention (BigBird-style) as
an in-scope variant.  AttnFuse currently expresses contiguous local
patterns (sliding-window) and dense+masked patterns (causal, ALiBi)
but does not yet support the global+local+random triple of BigBird,
which would require a sparse-block index buffer threaded through the
inner loop.  The IR is structurally ready for this---a new
\texttt{MaskKind.BLOCK\_SPARSE} can be added without touching the
fusion pass---but the codegen would need a per-row block-list traversal
...
93--96\% of SDPA's throughput---a modest penalty for kernel generality.
On the four (variant, sequence-length) cells that PyTorch's
\texttt{flex\_attention} can fuse but SDPA cannot, AttnFuse wins
\emph{12 of 16} head-to-head comparisons, with a clean sweep on
sliding-window. For the two variants SDPA cannot accelerate at all
(sliding-window, causal+ALiBi), AttnFuse provides up to
$56.5\times$ and $20.7\times$ speedup respectively by avoiding the
$\mathcal{O}(n^2)$ fallback. The strongest single result is on
compositions \texttt{flex\_attention} structurally cannot fuse: AttnFuse
is up to $2.59\times$ faster on \texttt{causal+RoPE} (\S\ref{sec:composition}).  The fused RoPE kernel eliminates a further 18--56\% latency
