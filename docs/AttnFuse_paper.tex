...
\paragraph{CUTLASS.}
NVIDIA CUTLASS~\cite{thakkar2023cutlass} is a C++ template library for
matrix multiplication and attention that exposes a tile-and-pipeline
abstraction similar in spirit to our tiled IR but operating at C++
expression-template granularity rather than as a typed Python dataflow
graph. CUTLASS provides explicit Hopper backends (WGMMA + TMA) which
is the architectural gap between our Triton kernels and SDPA's
underlying FlashAttention-2 CUDA implementation. The Triton-vs-CUTLASS
overhead documented in our backward results (\S\ref{sec:eval_backward})
is fundamentally because Triton emits standard MMA via tl.dot while
CUTLASS-backed kernels emit explicit WGMMA on Hopper and use TMA
descriptors for asynchronous memory operations.
